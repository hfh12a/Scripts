...
print combined_snr_ratio

fig, ax = pylab.subplots(1, 1, figsize=[15,10])
ax.hist(combined_snr_ratio[numpy.where(combined_snr_ratio < 0.95)[0]], 50)
ax.set_xlabel('Network SNR Ratio')
#fig.savefig(str(opt.output_directory)  + '/' + 'Network SNR Ratio, hist, below 0.95.png')
plt.close()

fig, ax = pylab.subplots(1, 1, figsize=[15,10])
ax.hist(stat_ratio[numpy.where(stat_ratio < 0.95)[0]], 50)
ax.set_xlabel('New SNR Ratio')
#fig.savefig(str(opt.output_directory)  + '/' + 'New SNR Ratio, hist, below 0.95.png')
plt.close()

fig, ax = pylab.subplots(1, 1, figsize=[15,10])
ax.hist(ifar_ratio, 50)
