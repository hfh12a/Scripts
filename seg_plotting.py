...
for param, name in zip(params, param_name):
    ...
    fig.savefig(str(opt.output_directory)  + '/' + name + ' vs. Time, lin.png')
    plt.close()
    ...
    fig.savefig(str(opt.output_directory)  + '/' + name + ' vs. Time, log.png')
    plt.close()
    ...
    fig.colorbar(ax.hexbin(time_comp, param), ax=ax)
    fig.savefig(str(opt.output_directory)  + '/' + name + ' vs. Chirp Mass, lin.png')
    plt.close()
    ...
    ax.set_xlim([numpy.min(chirp_comp), numpy.max(chirp_comp)])
    fig.colorbar(ax.hexbin(time_comp, param, yscale = 'log'), ax=ax)
    fig.savefig(str(opt.output_directory)  + '/' + name + ' vs. Chirp Mass, log.png')
    plt.close()
